...
def normalize_guitar_events(
    ...
    for i, item in enumerate(raw):
        ...
        if char_index is None:
            ...
            char_index = idx
        if char_index is None and item.get("bar") is not None:
...
def render_lyric_line(line: dict[str, Any]) -> str:
    ...
    for i, char in enumerate(clean_chars(lyrics)):
        ...
        if i in anchors:
            ...
            chunks.append(f'<span class="hit" data-chord="{html.escape(label, quote=True)}">{escaped}</span>')
        else:
    ...
    if not romaji:
        return f'<p class="lyric-line">{"".join(chunks)}</p>'
    lyric_html = f'<p class="lyric-line jp-line phrase-layout">{"".join(chunks)}</p>'
    return (
    ...
    <main class="page" aria-label="{title} Trail 和弦譜">
      <article class="chart">
        ...
        <figure class="matrix-figure">
          <img src="{html.escape(svg_name, quote=True)}" alt="吉他、人聲與交集 Trail 向量矩陣">
        </figure>
      </article>
